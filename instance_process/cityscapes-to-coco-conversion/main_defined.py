...
def convert_cityscapes_instance_only(data_dir):
    ...
    # Save the coordinates of the cropped region.
    model_name = os.path.basename(data_dir).split('_')[2]
    save_path = "../cityscapes-process/"
    ver = data_dir.split('/')[-1][-2:]
    name = f'changed_coordinated_instance_less_than_4_polygons_{model_name}_{ver}_tempt.json'
