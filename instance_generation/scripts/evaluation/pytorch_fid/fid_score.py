...
import sys
sys.path.append("/mmlabworkspace/WorkSpaces/danhnt/InstMask2Image/cityscapes_synthesis/evaluation/") 
...
def check_Nsample(image_name, sample_id: str=None):
    # print(image_name)
    if sample_id == None:
        sample_id = '000002'
    sample = image_name.split("_")[-2]
    # print(sample)
    if sample == sample_id:
        return False
    return True
...
def compute_statistics_of_path(path, model, batch_size, dims, device, num_workers=1):
    ...
    if isinstance(path, str):
        ...
        for city in new_cities:
            ...
            # path = pathlib.Path(city_path)
            image_list = os.listdir(city_path)
            image_list.sort()
            # breakpoint()
            files.extend(
                [os.path.join(city_path, file_name) for file_name in image_list if check_Nsample(file_name, sample_id='000002')]
            )
...
def calc_fid(path, batch_size, dims, num_workers=None, save_stats=False):
